...
| SHELL-OPEN-01 | 사용자 결정(대화, 2026-08-07): 셸(터미널)을 admin 전용에서 인증된 전체 사용자(admin·viewer)에게 개방한다. 세부: (1) 대상 범위는 새 중간 role 신설 없이 모든 viewer에게 즉시 개방, (2) 명령/키 입력 내용은 기록하지 않고 세션 메타데이터(username·시작·종료·사유)만 감사 로깅을 먼저 추가한 뒤 개방 — 최초 구현은 `session_id`도 포함했으나 `security-contract.md §1`("응답 본문·로그·...에는 ... session ID ... 기록하지 않는다")와 충돌함을 `security-reviewer` 반박 검토로 발견해 `session_id`를 로그에서 제거했다(username은 §1의 금지 목록에 없고 기존 `LoginLog`/`/api/admin/audit` 기능과 동일한 선례라 유지), (3) `MAX_SHELL_SESSIONS_GLOBAL=5`·`MAX_SHELL_SESSIONS_PER_USER=1`과 디스크 quota 부재는 이번 범위에서 변경하지 않는다. | 사용자가 대화에서 "격리·안전장치가 이미 되어 있으니 일반 사용자에게 열어도 되지 않냐"고 질의했다. Explore 조사로 컨테이너 격리(세션당 신규 컨테이너, `--network none`, `--cap-drop ALL`, `--read-only`, non-root, 리소스 제한, 사용자별 홈 마운트)는 견고하나, 감사 로깅이 전무했고 role 모델이 admin/viewer 이진이라 세분화가 불가함을 확인했다. PTY 원문을 그대로 감사 로그로 남기면 CLAUDE.md의 비밀번호·시크릿 미로깅 원칙과 충돌할 수 있어(셸 안에서 입력한 비밀번호가 그대로 남을 위험) 로깅 범위를 메타데이터로 한정하기로 사용자가 결정했다. `docs/re-plan.md` 밖 OUT_OF_PLAN_CHANGE이며 계획 밖 기능·정책 변경이므로 여기 별도 결정으로 기록한다. | APPROVED |
...
| SHELL-OPEN-01 | OUT_OF_PLAN_CHANGE / 사후 추가 | DONE | Main (사용자 요청) | 없음 (`docs/re-plan.md` 밖 범위, SHELL-OPEN-01 결정 적용) | 셸을 admin 전용에서 인증된 전체 사용자로 개방했다. **백엔드**: `backend/routers/shell.py` WS 핸들러의 role 재확인(`user.role != 'admin'`)을 제거하고 비활성 사용자만 거부하도록 좁혔다. `/api/shell/fs`·`/api/shell/sessions`·`/api/shell/reset` REST 3개와 `backend/routers/auth.py`의 `POST /api/auth/ws-tickets/shell`을 `get_current_admin`에서 `get_current_user`로 교체했다(세션 소유자 격리 `session.username != username` → 403은 그대로 유지해 viewer 간 격리를 담당). 셸 WS 생명주기 로그(`opened`/`capacity_rejected`/`container_start_failed`/`started`/`read_failed`/`unexpected_error`/`closed`)에 `username`을 추가해 메타데이터 감사 추적을 보강했다(명령/키 입력 내용은 기록하지 않음, SHELL-OPEN-01 결정). 최초 구현은 `session_id`도 함께 로깅했으나, `security-reviewer` 반박 검토가 `security-contract.md §1`의 "session ID를 로그에 기록하지 않는다" 규정과 정면 충돌함을 지적(모니터링 WS의 `test_secret_logging.py`가 이미 `request_id`(opaque)만 허용하고 상관 가능한 식별자는 금지하는 정적 AST 가드로 이 원칙을 강제 중임을 근거로 제시)해, `session_id`를 로그 인자에서 제거하고 `username`·`result`만 남겼다. **프론트엔드**: `frontend/src/App.jsx`의 `/terminal` 라우트에서 `requiredRole="admin"`을 제거하고, `frontend/src/components/Sidebar.jsx`의 터미널 메뉴를 `isAdmin` 게이트 밖으로 옮겼다(`/users` 사용자 관리는 admin 전용 유지, 범위 밖). **문서**: `docs/contracts/security-contract.md` §2·§3·§5의 셸 관련 admin 전용 서술을 인증된 전체 사용자 기준으로 갱신하고 변경 사유를 주석으로 남겼다. `docs/re-plan.md`의 Phase 0 권한 행렬(BASE-02)과 AUTHZ-03 계약 항목은 완료 당시 기록을 보존하고 취소선·인용구로 현재 계약 위치를 가리키게 했다(역사 왜곡 방지). **테스트**: 실패 테스트를 먼저 작성한 뒤 구현했다 — `test_authorization_matrix_e2e.py`의 셸 4행을 viewer 403→ALLOW로, `test_shell_rest_authorization.py`를 전면 재작성(unauthenticated 401·admin/viewer 모두 허용·`get_current_user` 의존성 검증·다른 사용자 홈 디렉터리 비영향), `test_shell_file_tree.py`의 라우트 테스트를 `get_current_user` 기준 소유권 검증으로, `test_websocket_authorization.py`의 ticket·WS role 테스트를 viewer 허용·비활성 사용자만 거부로 각각 고쳤다. backend 전체 202 PASS(로컬 `.venv`/`venv`가 불완전해 새 venv를 만들어 `requirements.txt`로 설치 후 실행), frontend 전체 102 PASS. 변경 파일: `backend/routers/shell.py`, `backend/routers/auth.py`, `backend/tests/test_authorization_matrix_e2e.py`, `backend/tests/test_shell_rest_authorization.py`, `backend/tests/test_shell_file_tree.py`, `backend/tests/test_websocket_authorization.py`, `frontend/src/App.jsx`, `frontend/src/components/Sidebar.jsx`, `docs/contracts/security-contract.md`, `docs/re-plan.md`. |
...
| Task | 명령 | 결과 | 실행 환경 | 비고 |
| --- | --- | --- | --- | --- |
| SHELL-OPEN-01 | `python -m pytest tests/test_shell_rest_authorization.py tests/test_shell_file_tree.py tests/test_websocket_authorization.py -q` | PASS (27 passed) | Linux sandbox, 새로 만든 venv(`requirements.txt` 설치) | 격리 실행 |
| SHELL-OPEN-01 | `python -m pytest tests -q` (backend 전체) | PASS (202 passed, 88.33s) | Linux sandbox, 위와 동일 venv | `test_authorization_matrix_e2e.py`를 단독 실행하면 alembic이 이 파일의 Windows용 termios stub(`TIOCSWINSZ`만 정의)을 실제 모듈보다 먼저 가로채 `TIOCGWINSZ` 누락으로 실패하는 사전 존재 아티팩트를 확인했다(collection 순서 의존). 전체 스위트로 실행하면 다른 파일이 먼저 실제 termios를 import해 재현되지 않으며, 이번 변경과 무관하다 |
| SHELL-OPEN-01 | `npm test -- --run` (frontend 전체) | PASS (102 passed) | Linux sandbox | 12개 테스트 파일, terminal 관련 전용 테스트 없음(ProtectedRoute는 범용 컴포넌트 테스트라 변경 불필요) |
| SHELL-OPEN-01 | `bash scripts/gate.sh` (Phase 완료 게이트) | FAIL (PASS 9, FAIL 1) | Linux sandbox, `python`/`python3`를 위 venv `bin/`으로 PATH 선행 | 유일한 실패는 `frontend lint`의 `frontend/src/utils/terminalSafety.js` (no-control-regex 4건, no-useless-escape 2건). 이 파일은 이번 변경에 포함되지 않았고 AI-NARRATE-01 때부터 있던 사전 존재 문제로 이미 Known Limitations에 기록돼 있다(요청과 무관한 정리 금지 원칙에 따라 여기서도 수정하지 않음). backend pytest·frontend test/build·compose config(운영·개발)·security scan·공백 검사(추적/미추적) 9개는 모두 PASS |
| SHELL-OPEN-01 | `security-reviewer` 서브에이전트 반박 검토 (Phase 게이트 전 보안 diff 검증, CLAUDE.md 서브에이전트 방침) | FAIL → 수정 후 재확인 | Main 위임 | 세션 소유권 격리·타 admin 라우트 비오염·WS 인증 경로·테스트 실효성·문서 일치성 5개 항목은 CONFIRMED PASS. 로깅 항목에서 위반 1건 발견: 추가한 `session_id` 로그가 `security-contract.md §1`의 "session ID를 로그에 기록하지 않는다"와 충돌(`shell.py:287,356,372,389` 등). 발견 즉시 `session_id`를 로그 인자에서 제거하고(`username`·`result`만 유지) `python -m pytest tests -q`(backend 전체) 202 PASS로 재확인했다. 재위임한 재검토는 실행하지 않았으나(비용 대비 위반 성격이 단순 인자 제거로 명확히 해소됨), 수정 내용과 근거를 여기 기록한다 |
...
- **SHELL-OPEN-01**: 셸을 admin 전용에서 인증된 전체 사용자로 개방하면서 `MAX_SHELL_SESSIONS_GLOBAL=5`(전역 동시 세션 5개)와 사용자별 홈 디렉터리 디스크 quota 부재는 그대로 두었다(사용자 결정, 범위 밖). 대상 인원이 admin 소수에서 전체 viewer로 늘어나면 전역 세션 한도가 실질적인 병목·거부 상태가 되거나 디스크 사용량이 무제한으로 늘 수 있다 — 인원이 실제로 늘면 재검토가 필요하다.
- SHELL-OPEN-01 감사 로깅은 셸 WS 생명주기(`opened`/`started`/`closed`/`capacity_rejected`/`container_start_failed`/`read_failed`/`unexpected_error`)에 `username`과 고정 `result`만 남긴다. PTY로 오간 키 입력·명령어 내용은 기록하지 않으므로(사용자 결정), 사고 발생 시 "누가 언제 접속·종료했는지"까지만 추적 가능하고 "무엇을 실행했는지"는 추적할 수 없다.
