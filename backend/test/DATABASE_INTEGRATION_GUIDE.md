> ## ⚠️ 과거 기록 — 현재 동작이 아닙니다
>
> 이 문서는 **2026-07-29 이전** 구현 시점의 작업 기록이며 현재 자동 검사 결과가
> 아닙니다. 이후 `DB-01`이 스키마 관리 방식을 바꿔서 다음이 달라졌습니다.
>
> | 이 문서의 전제 | 현재 구현 |
> |---|---|
> | 시작 시 임의 `ALTER TABLE`로 컬럼 보정 | **Alembic 마이그레이션**(`alembic upgrade head`)을 시작 시 실행, 실패하면 기동 중단 |
> | 스키마 버전 관리 없음 | `backend/migrations/`의 리비전으로 관리 |
> | 완료 상태 서술 | 현재 결과는 `python -m pytest backend/tests` 또는 `bash scripts/gate.sh` 실행 결과를 근거로 삼을 것 |
>
> 현재 DB 운영·백업·복구 절차는 `docs/db-operations.md`, 자동 검증은
> `backend/tests/test_db_migrations.py`를 보세요.
